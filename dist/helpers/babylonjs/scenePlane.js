...
export function get_scenePlaneUtils(storeHelpers, prendyStartOptions) {
    ...
    function getPositionOnPlane(theMesh) {
        ...
        return Vector3.Project(new Vector3(theMesh.position.x, theMesh.position.y + Y_OFFSET, theMesh.position.z), Matrix.Identity(), currentCamera
            ...
            .multiply(getProjectionMatrixCustomSize(currentCamera, globalRefs.backdropSize)), currentCamera.viewport.toGlobal(globalRefs.backdropSize.width, globalRefs.backdropSize.height));
    }
    function getPlanePositionNotOverEdges(planePos, useGoal) {
        ...
        if (isOverRightEdge)
            newPlanePos.x = maxShiftX;
        if (isOverLeftEdge)
            newPlanePos.x = -maxShiftX;
        if (isOverBottomEdge)
            newPlanePos.y = maxShiftY;
        if (isOverTopEdge)
            newPlanePos.y = -maxShiftY;
