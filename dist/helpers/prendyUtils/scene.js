...
export function get_sceneStoryUtils(storeHelpers) {
    ...
    function doWhenNowPlaceChanges(checkingPlaceName, callback) {
        const { nowPlaceName } = getState().global.main;
    ...
    function doWhenPlaceFullyLoaded(checkingPlaceName, callback) {
        const { nowPlaceName } = getState().global.main;
        const initialNowPlaceName = getState().global.main.nowPlaceName;
    ...
    async function waitForPlaceFullyLoaded(checkingPlaceName) {
        return new Promise((resolve) => {
            doWhenPlaceFullyLoaded(checkingPlaceName, resolve);
        });
    }
    async function waitForNowPlaceToChange(checkingPlaceName) {
        return new Promise((resolve) => {
            doWhenNowPlaceChanges(checkingPlaceName, resolve);
        });
    }
    async function waitForNowCamToChange(checkingCamName) {
        return new Promise((resolve) => {
