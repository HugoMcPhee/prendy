...
export function get_globalVideoRules(storeHelpers, _prendyStores, prendyOptions, prendyAssets) {
    const { getRefs, getState, makeRules, setState, onNextTick } = storeHelpers;
    const { getSliceForPlace, getSliceVidVideo, getSliceVidWaitingVideo, checkForVideoLoop, checkIfVideoAlreadyChanging, } = get_sliceVidUtils(storeHelpers, prendyOptions, prendyAssets);
    const { getSafeSegmentName, updateTexturesForNowCamera, updateNowStuffWhenSliceChanged } = get_cameraChangeUtils(storeHelpers, prendyOptions, prendyAssets);
