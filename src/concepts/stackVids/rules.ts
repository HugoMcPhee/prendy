...
export function makeStackVidRules<ConceptoFuncs extends GameyConceptoFuncs>(
  ...
  return makeRules((addItemEffect) => ({
    ...
    rulesForSettingNewVideoStates: addItemEffect({
      onItemEffect({ newValue: vidState, itemState, itemName }) {
        ...
        function setItemState(newState: Partial<ItemState<"stackVids">>) {
          setState({ stackVids: { [itemName]: newState } });
        }

        function setVidState(vidState: StackVidState) {
                  ...
                  onNextTick(() => {
                    ...
                    if (newState.autoplay) {
                      doWhenStackVidPlay(
                        ...
                        () => {
                          const { vidAId, vidBId } =
                            getState().stackVids[itemName];
                          if (vidAId === null || vidBId === null) return;
                          const vidATime =
                            safeVidRefs[vidAId].videoElement?.currentTime;
                          const vidBTime =
                            safeVidRefs[vidBId].videoElement?.currentTime;
