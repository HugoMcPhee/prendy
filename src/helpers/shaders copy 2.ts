...
gl_FragColor = sceneOnBackdropColorSmoother;
    ...
    #define CUSTOM_VERTEX_MAIN_BEGIN
      ...
      vec2 transformPosition = planePos;  
      vec2 scale = stretchSceneAmount;  
      float rotation = 0.0;   // degrees
      float r = rotation/180.0*3.14159; // radians
      ...
      mat3 mt = makeTranslation( transformPosition );
      mat3 mr = makeRotation( r ); 
      mat3 ms = makeScale( 1.0/scale ); 
      mat3 msVideo = makeScale( 1.0/stretchVideoAmount ); 
    ...
    //  note might not be defined automatically
    ...
    void main(void) {
      ...
      // ------------------------------------------
      // Calculating UV Transforms
      // ------------------------------------------
    
      vec2 transform_position = planePos;
      vec2 transform_scale = stretchSceneAmount;
      float transform_rotation = 0.0;   // degrees
      float r = transform_rotation / 180.0 * 3.14159; // radians
    
      vec2 target_res = vec2(1.0, 1.0);
    
      mat3 mt = makeTranslation(transform_position);
      mat3 mr = makeRotation(r);
      mat3 ms = makeScale(1.0 / transform_scale);
      mat3 msVideo = makeScale(1.0 / stretchVideoAmount); 
      ...
      // this one prevents the weird white outlines
      vec4 sceneOnBackdropColorSmoother = mix(backdropTexture, sceneOnBackdropColor, color.w);
      // amount for color and amount for backdrop
      // should both be a number combined make 1
    
      vec4 newTextureSampler = sceneOnBackdropColorSmoother;
      ...
      // Vertex
      // vUV_fxaa = (position * madd + madd);
      vUV_fxaa = newUv;
    
      sampleCoordS = vUV_fxaa + vec2(0.0, 1.0) * texelSize;
      sampleCoordE = vUV_fxaa + vec2(1.0, 0.0) * texelSize;
      sampleCoordN = vUV_fxaa + vec2(0.0, -1.0) * texelSize;
      sampleCoordW = vUV_fxaa + vec2(-1.0, 0.0) * texelSize;
    
      sampleCoordNW = vUV_fxaa + vec2(-1.0, -1.0) * texelSize;
      sampleCoordSE = vUV_fxaa + vec2(1.0, 1.0) * texelSize;
      sampleCoordNE = vUV_fxaa + vec2(1.0, -1.0) * texelSize;
      sampleCoordSW = vUV_fxaa + vec2(-1.0, 1.0) * texelSize;
      ...
      vec4 rgbyM = TEXTUREFUNC(textureSampler, newUv, 0.0);
