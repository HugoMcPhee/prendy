...
>(concepFuncs: ConcepFuncs, prendyStartOptions: PrendyOptions) {
  ...
  function getProjectionMatrixCustomSize(
    theCamera: Camera,
    theSize: { width: number; height: number }
  ) {
  ...
  function getPositionOnPlane(theMesh: AbstractMesh) {
    ...
    return Vector3.Project(
      ...
      currentCamera
        ...
        .multiply(
          getProjectionMatrixCustomSize(
            currentCamera,
            globalRefs.backdropImageSize
          )
        ),
      currentCamera.viewport.toGlobal(
        globalRefs.backdropImageSize.width,
        globalRefs.backdropImageSize.height
      )
    );
  }

  function updatePlanePositionToFocusOnMesh({
    meshRef,
    instant,
  }: {
    ...
    const safePlanePosition = getSafePlanePositionFocusedOnPointOnPlain(
      meshSafePointOnPlane
    );
  ...
  function convertPointOnPlaneToPointOnScreen({
    ...
    const unmovedPointOnScreen = convertPointOnPlaneToUnmovedPointOnScreen(
      safePointOnPlane
    );
  ...
  function getScenePlaneOverScreenEdgesAmount(newPosition: {
    ...
    const planePositionOnScreenVector = getScenePlanePositionOnScreen(
  ...
  function getSafePlanePositionFocusedOnPointOnPlain(pointOnPlane: Point2D) {
    ...
    const scaledPointOnPlane = convertPointOnPlaneToUnmovedPointOnScreen(
      pointOnPlane
    );
    ...
    const amountOverEdges = getScenePlaneOverScreenEdgesAmount(
      newPlanePosition
    );
